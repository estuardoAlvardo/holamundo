...
$insertarCuestionario->bindparam(':intento',$_POST['intento']);
  ...
  while(@$row1=$obtenerCuestionario->fetch(PDO::FETCH_ASSOC)){
    ...
    if($row1['respuestaCorrecta']==0){
       ...
       similar_text($_POST['input'.$row1['idItem']], $row1['matrizComparativa'],$var);

       
       $reconocimiento=':rInput'.$j;
       
        number_format($var,0);
       $insertarCuestionario->bindparam($reconocimiento, $var);
